...
export default async function VoicePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'voice' });

  const sections = ['overview', 'trends', 'ai', 'business', 'global'] as const;
      ...
      <section className="pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="space-y-8">
          {sections.map((section) => (
            <div key={section} className="p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all">
              <h2 className="text-2xl font-bold text-white mb-4">{t(`${section}.title`)}</h2>
              <p className="text-gray-300 leading-relaxed">{t(`${section}.description`)}</p>
            </div>
          ))}
        </div>
